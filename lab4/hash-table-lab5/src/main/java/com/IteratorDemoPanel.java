package com;

import javax.swing.*;
import javax.swing.border.TitledBorder;
import javax.swing.table.DefaultTableModel;
import java.awt.*;
import java.util.List;
import java.util.NoSuchElementException;

/**
 * Панель для демонстрации работы итератора хеш-таблицы
 * ВАРИАНТ №6: ключ - строка (заглавные латинские буквы A-Z)
 *
 * Использует улучшенный HashTableIterator, поддерживающий:
 * - Корректное удаление элементов через таблицу
 * - Сохранение позиции итератора при удалении
 * - Возможность удаления через итератор (Iterator.remove())
 */
public class IteratorDemoPanel extends JPanel {

    private HashTable<String, String> hashTable;
    private JTable iteratorTable;
    private DefaultTableModel tableModel;
    private JTextArea iteratorOutput;
    private JButton refreshButton;
    private JButton forwardButton;
    private JButton resetButton;
    private JButton traverseButton;
    private JButton removeViaIteratorButton;
    private JLabel currentElementLabel;
    private JLabel iteratorStatusLabel;

    private HashTableIterator<String, String> currentIterator;
    private HashTable.Entry<String, String> currentElement;
    private String currentKey;
    private int currentIndex;

    private static final Color COLOR_SUCCESS = new Color(46, 204, 113);
    private static final Color COLOR_WARNING = new Color(241, 196, 15);
    private static final Color COLOR_ERROR = new Color(231, 76, 60);
    private static final Color COLOR_INFO = new Color(52, 152, 219);

    public IteratorDemoPanel() {
        setLayout(new BorderLayout(10, 10));
        setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));

        this.hashTable = null;
        this.currentIterator = null;
        this.currentElement = null;
        this.currentKey = null;
        this.currentIndex = -1;

        initComponents();
    }

    private void initComponents() {
        // Верхняя панель - элементы управления итератором
        JPanel controlPanel = new JPanel(new BorderLayout(10, 5));
        controlPanel.setBorder(BorderFactory.createTitledBorder(
                BorderFactory.createLineBorder(COLOR_INFO, 2),
                "Управление итератором",
                TitledBorder.LEFT,
                TitledBorder.TOP,
                new Font("Arial", Font.BOLD, 12),
                COLOR_INFO
        ));

        JPanel buttonPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 10, 5));
        refreshButton = new JButton("Обновить итератор (begin())");
        refreshButton.setToolTipText("Устанавливает итератор на первый элемент коллекции");
        forwardButton = new JButton("Переход к следующему (++)");
        forwardButton.setToolTipText("Перемещает итератор к следующему элементу");
        resetButton = new JButton("Сбросить итератор");
        resetButton.setToolTipText("Сбрасывает итератор в начальное состояние");
        removeViaIteratorButton = new JButton("🗑 Удалить через итератор (Iterator.remove())");
        removeViaIteratorButton.setToolTipText("Удаляет текущий элемент через итератор (стандартный метод Iterator)");

        refreshButton.setBackground(COLOR_INFO);
        refreshButton.setForeground(Color.WHITE);
        refreshButton.setFocusPainted(false);

        forwardButton.setBackground(COLOR_SUCCESS);
        forwardButton.setForeground(Color.WHITE);
        forwardButton.setFocusPainted(false);

        resetButton.setBackground(COLOR_WARNING);
        resetButton.setForeground(Color.WHITE);
        resetButton.setFocusPainted(false);

        removeViaIteratorButton.setBackground(new Color(192, 57, 43));
        removeViaIteratorButton.setForeground(Color.WHITE);
        removeViaIteratorButton.setFocusPainted(false);

        refreshButton.addActionListener(e -> resetIterator());
        forwardButton.addActionListener(e -> nextElement());
        resetButton.addActionListener(e -> resetIterator());
        removeViaIteratorButton.addActionListener(e -> removeCurrentViaIterator());

        buttonPanel.add(refreshButton);
        buttonPanel.add(forwardButton);
        buttonPanel.add(resetButton);
        buttonPanel.add(removeViaIteratorButton);

        // Панель статуса итератора
        JPanel statusPanel = new JPanel(new GridLayout(3, 1, 5, 5));
        statusPanel.setBorder(BorderFactory.createEmptyBorder(5, 5, 5, 5));

        currentElementLabel = new JLabel("Текущий элемент: (нет)");
        currentElementLabel.setFont(new Font("Monospaced", Font.PLAIN, 12));

        iteratorStatusLabel = new JLabel("Состояние итератора: не установлен");
        iteratorStatusLabel.setFont(new Font("Monospaced", Font.PLAIN, 12));

        JLabel hintLabel = new JLabel("Подсказка: удаление через итератор (Iterator.remove()) корректно обновляет позицию");
        hintLabel.setFont(new Font("Arial", Font.ITALIC, 10));
        hintLabel.setForeground(Color.GRAY);

        statusPanel.add(currentElementLabel);
        statusPanel.add(iteratorStatusLabel);
        statusPanel.add(hintLabel);

        controlPanel.add(buttonPanel, BorderLayout.NORTH);
        controlPanel.add(statusPanel, BorderLayout.CENTER);

        // Центральная панель - таблица с данными
        JPanel dataPanel = new JPanel(new BorderLayout(5, 5));
        dataPanel.setBorder(BorderFactory.createTitledBorder(
                "Данные в хеш-таблице (с отображением хеш-индексов)"
        ));

        String[] columns = {"№", "Индекс", "Ключ (k)", "Значение (data)", "k' (преобр.)", "Хеш-индекс"};
        tableModel = new DefaultTableModel(columns, 0) {
            @Override
            public boolean isCellEditable(int row, int column) {
                return false;
            }
        };
        iteratorTable = new JTable(tableModel);
        iteratorTable.setFont(new Font(Font.MONOSPACED, Font.PLAIN, 12));
        iteratorTable.setRowHeight(25);
        iteratorTable.getTableHeader().setFont(new Font("Arial", Font.BOLD, 12));

        iteratorTable.setSelectionBackground(new Color(52, 152, 219, 100));
        iteratorTable.setSelectionForeground(Color.BLACK);

        iteratorTable.getColumnModel().getColumn(0).setMaxWidth(40);
        iteratorTable.getColumnModel().getColumn(1).setMaxWidth(60);
        iteratorTable.getColumnModel().getColumn(2).setPreferredWidth(120);
        iteratorTable.getColumnModel().getColumn(3).setPreferredWidth(150);
        iteratorTable.getColumnModel().getColumn(4).setPreferredWidth(150);
        iteratorTable.getColumnModel().getColumn(5).setMaxWidth(100);

        JScrollPane tableScroll = new JScrollPane(iteratorTable);
        tableScroll.setPreferredSize(new Dimension(750, 200));
        dataPanel.add(tableScroll, BorderLayout.CENTER);

        // Панель управления данными
        JPanel dataControlPanel = new JPanel(new GridBagLayout());
        dataControlPanel.setBorder(BorderFactory.createTitledBorder(
                "Управление данными"
        ));
        GridBagConstraints gbc = new GridBagConstraints();
        gbc.insets = new Insets(5, 5, 5, 5);
        gbc.fill = GridBagConstraints.HORIZONTAL;

        JLabel keyLabel = new JLabel("Ключ (A-Z):");
        keyLabel.setFont(new Font("Arial", Font.BOLD, 11));
        JTextField keyField = new JTextField(15);
        keyField.setToolTipText("Введите строку из заглавных латинских букв (A-Z)");

        JLabel valueLabel = new JLabel("Значение:");
        valueLabel.setFont(new Font("Arial", Font.BOLD, 11));
        JTextField valueField = new JTextField(15);

        JButton addButton = new JButton("➕ Добавить");
        addButton.setBackground(COLOR_SUCCESS);
        addButton.setForeground(Color.WHITE);
        addButton.setFocusPainted(false);

        JButton deleteByKeyButton = new JButton("🗑 Удалить по ключу");
        deleteByKeyButton.setBackground(COLOR_ERROR);
        deleteByKeyButton.setForeground(Color.WHITE);
        deleteByKeyButton.setFocusPainted(false);

        JButton clearButton = new JButton("❌ Очистить таблицу");
        clearButton.setBackground(COLOR_WARNING);
        clearButton.setForeground(Color.WHITE);
        clearButton.setFocusPainted(false);

        JButton loadSampleButton = new JButton("📋 Загрузить образец");
        loadSampleButton.setBackground(COLOR_INFO);
        loadSampleButton.setForeground(Color.WHITE);
        loadSampleButton.setFocusPainted(false);

        JLabel infoLabel = new JLabel(
                "Допустимые символы: A-Z | Макс. длина: " + KeyTransformer.getMaxKeyLength()
        );
        infoLabel.setFont(new Font("Arial", Font.ITALIC, 10));
        infoLabel.setForeground(Color.GRAY);

        addButton.addActionListener(e -> {
            if (hashTable == null) return;
            try {
                String key = keyField.getText().trim().toUpperCase();
                String value = valueField.getText().trim();

                if (key.isEmpty() || value.isEmpty()) {
                    showMessageDialog("Заполните оба поля", "Ошибка", JOptionPane.ERROR_MESSAGE);
                    return;
                }

                if (!KeyTransformer.isValidKey(key)) {
                    showMessageDialog("Недопустимый ключ. Используйте A-Z", "Ошибка", JOptionPane.ERROR_MESSAGE);
                    return;
                }

                if (hashTable.insert(key, value)) {
                    updateTableDisplay();
                    syncIteratorPosition();
                    appendToOutput("✓ Добавлен: " + key + " → " + value + "\n");
                    keyField.setText("");
                    valueField.setText("");
                } else {
                    showMessageDialog("Ключ уже существует", "Ошибка", JOptionPane.ERROR_MESSAGE);
                }
            } catch (Exception ex) {
                showMessageDialog(ex.getMessage(), "Ошибка", JOptionPane.ERROR_MESSAGE);
            }
        });

        deleteByKeyButton.addActionListener(e -> {
            if (hashTable == null) return;
            String input = JOptionPane.showInputDialog(this, "Введите ключ для удаления:");
            if (input != null) {
                String key = input.trim().toUpperCase();
                boolean isCurrent = (currentElement != null && currentElement.getKey().equals(key));

                if (hashTable.delete(key)) {
                    updateTableDisplay();
                    if (isCurrent) {
                        // После удаления текущего элемента через таблицу,
                        // синхронизируем итератор, чтобы он указывал на следующий элемент
                        syncIteratorAfterExternalDelete();
                        appendToOutput("🗑 Удалён текущий элемент: " + key + " (итератор обновлён)\n");
                    } else {
                        syncIteratorPosition();
                        appendToOutput("🗑 Удалён элемент: " + key + "\n");
                    }
                    showMessageDialog("Элемент удалён", "Успех", JOptionPane.INFORMATION_MESSAGE);
                } else {
                    showMessageDialog("Элемент не найден", "Ошибка", JOptionPane.WARNING_MESSAGE);
                }
            }
        });

        clearButton.addActionListener(e -> {
            if (hashTable == null) return;
            int confirm = JOptionPane.showConfirmDialog(this, "Очистить таблицу?", "Подтверждение", JOptionPane.YES_NO_OPTION);
            if (confirm == JOptionPane.YES_OPTION) {
                hashTable.clear();
                updateTableDisplay();
                currentIterator = null;
                currentElement = null;
                currentKey = null;
                currentIndex = -1;
                updateIteratorStatus();
                appendToOutput("=== Таблица очищена ===\n");
            }
        });

        loadSampleButton.addActionListener(e -> loadSampleData());

        gbc.gridx = 0; gbc.gridy = 0;
        dataControlPanel.add(keyLabel, gbc);
        gbc.gridx = 1;
        dataControlPanel.add(keyField, gbc);
        gbc.gridx = 2;
        dataControlPanel.add(valueLabel, gbc);
        gbc.gridx = 3;
        dataControlPanel.add(valueField, gbc);

        gbc.gridx = 0; gbc.gridy = 1;
        dataControlPanel.add(addButton, gbc);
        gbc.gridx = 1;
        dataControlPanel.add(deleteByKeyButton, gbc);
        gbc.gridx = 2;
        dataControlPanel.add(clearButton, gbc);
        gbc.gridx = 3;
        dataControlPanel.add(loadSampleButton, gbc);

        gbc.gridx = 0; gbc.gridy = 2;
        gbc.gridwidth = 4;
        dataControlPanel.add(infoLabel, gbc);

        dataPanel.add(dataControlPanel, BorderLayout.SOUTH);

        // Нижняя панель - вывод результатов обхода
        JPanel outputPanel = new JPanel(new BorderLayout());
        outputPanel.setBorder(BorderFactory.createTitledBorder("Результат обхода коллекции"));

        iteratorOutput = new JTextArea(10, 60);
        iteratorOutput.setEditable(false);
        iteratorOutput.setFont(new Font(Font.MONOSPACED, Font.PLAIN, 12));
        iteratorOutput.setBackground(new Color(250, 250, 250));
        JScrollPane outputScroll = new JScrollPane(iteratorOutput);
        outputScroll.setPreferredSize(new Dimension(750, 180));

        traverseButton = new JButton("▶ Выполнить полный обход (foreach)");
        traverseButton.setBackground(new Color(155, 89, 182));
        traverseButton.setForeground(Color.WHITE);
        traverseButton.setFocusPainted(false);
        traverseButton.addActionListener(e -> performFullTraversal());

        JPanel traversePanel = new JPanel(new FlowLayout(FlowLayout.RIGHT));
        traversePanel.add(traverseButton);

        outputPanel.add(outputScroll, BorderLayout.CENTER);
        outputPanel.add(traversePanel, BorderLayout.SOUTH);

        JSplitPane splitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT);
        splitPane.setTopComponent(dataPanel);
        splitPane.setBottomComponent(outputPanel);
        splitPane.setResizeWeight(0.5);
        splitPane.setDividerLocation(300);

        add(controlPanel, BorderLayout.NORTH);
        add(splitPane, BorderLayout.CENTER);
    }

    /**
     * Синхронизация позиции итератора после внешних изменений
     */
    private void syncIteratorPosition() {
        if (hashTable == null || hashTable.size() == 0) {
            currentIterator = null;
            currentElement = null;
            currentKey = null;
            currentIndex = -1;
            updateIteratorStatus();
            return;
        }

        // Если есть текущий ключ, пытаемся найти его в таблице
        if (currentKey != null && hashTable.containsKey(currentKey)) {
            currentIndex = hashTable.getIndexByKey(currentKey);
            if (currentIndex >= 0) {
                currentElement = hashTable.getEntryByIndex(currentIndex);
                // Создаём новый итератор, установленный на нужную позицию
                currentIterator = hashTable.getDetailedIterator();
                // Проматываем до нужного индекса
                while (currentIterator.hasNext() && currentIterator.getCurrentIndex() < currentIndex) {
                    currentIterator.next();
                }
                if (currentIterator.hasNext() && currentIterator.getCurrentIndex() == currentIndex) {
                    currentElement = currentIterator.next();
                }
            } else {
                resetIterator();
            }
        } else {
            resetIterator();
        }
        updateIteratorStatus();
        highlightCurrentRow();
    }

    /**
     * Синхронизация после удаления текущего элемента через таблицу
     * Итератор должен перейти к следующему элементу
     */
    private void syncIteratorAfterExternalDelete() {
        if (hashTable == null || hashTable.size() == 0) {
            currentIterator = null;
            currentElement = null;
            currentKey = null;
            currentIndex = -1;
            updateIteratorStatus();
            return;
        }

        // Находим следующий активный индекс после удалённого
        int nextIdx = hashTable.getNextActiveIndex(currentIndex);
        if (nextIdx >= 0) {
            currentIndex = nextIdx;
            currentElement = hashTable.getEntryByIndex(currentIndex);
            currentKey = currentElement != null ? currentElement.getKey() : null;

            // Создаём новый итератор и устанавливаем на нужную позицию
            currentIterator = hashTable.getDetailedIterator();
            while (currentIterator.hasNext() && currentIterator.getCurrentIndex() < currentIndex) {
                currentIterator.next();
            }
            if (currentIterator.hasNext() && currentIterator.getCurrentIndex() == currentIndex) {
                currentElement = currentIterator.next();
            }
        } else {
            // Если нет следующего элемента, итератор становится end()
            currentIterator = null;
            currentElement = null;
            currentKey = null;
            currentIndex = -1;
        }

        updateIteratorStatus();
        highlightCurrentRow();
    }

    /**
     * Удаление текущего элемента через итератор (Iterator.remove())
     */
    private void removeCurrentViaIterator() {
        if (currentIterator == null) {
            showMessageDialog("Итератор не инициализирован", "Ошибка", JOptionPane.WARNING_MESSAGE);
            return;
        }

        if (currentElement == null) {
            showMessageDialog("Нет текущего элемента для удаления", "Ошибка", JOptionPane.WARNING_MESSAGE);
            return;
        }

        String keyToDelete = currentElement.getKey();
        int confirm = JOptionPane.showConfirmDialog(this,
                "Удалить текущий элемент через Iterator.remove()?\n" +
                        "Ключ: " + keyToDelete + "\nЗначение: " + currentElement.getValue(),
                "Подтверждение удаления",
                JOptionPane.YES_NO_OPTION);

        if (confirm == JOptionPane.YES_OPTION) {
            try {
                currentIterator.remove();
                updateTableDisplay();

                // После remove() итератор должен указывать на следующий элемент
                if (currentIterator.hasNext()) {
                    currentElement = currentIterator.next();
                    currentKey = currentElement.getKey();
                    currentIndex = hashTable.getIndexByKey(currentKey);
                    appendToOutput("🗑 Удалён через итератор: " + keyToDelete +
                            " (итератор перешёл к следующему)\n");
                } else {
                    currentElement = null;
                    currentKey = null;
                    currentIndex = -1;
                    appendToOutput("🗑 Удалён через итератор: " + keyToDelete +
                            " (итератор достиг end())\n");
                }

                updateIteratorStatus();
                highlightCurrentRow();
                showMessageDialog("Элемент удалён через итератор", "Успех", JOptionPane.INFORMATION_MESSAGE);

            } catch (IllegalStateException e) {
                showMessageDialog(e.getMessage(), "Ошибка", JOptionPane.ERROR_MESSAGE);
            }
        }
    }

    private void resetIterator() {
        if (hashTable == null || hashTable.size() == 0) {
            currentIterator = null;
            currentElement = null;
            currentKey = null;
            currentIndex = -1;
            updateIteratorStatus();
            appendToOutput("⚠ Итератор не может быть инициализирован\n");
            return;
        }

        currentIterator = hashTable.getDetailedIterator();
        if (currentIterator.hasNext()) {
            currentElement = currentIterator.next();
            currentKey = currentElement.getKey();
            currentIndex = hashTable.getIndexByKey(currentKey);
            updateIteratorStatus();
            highlightCurrentRow();
            appendToOutput("✓ Итератор установлен на первый элемент (begin())\n");
        } else {
            currentElement = null;
            currentKey = null;
            currentIndex = -1;
            updateIteratorStatus();
            appendToOutput("⚠ Итератор достиг конца коллекции (end())\n");
        }
    }

    private void nextElement() {
        if (currentIterator == null) {
            showMessageDialog("Итератор не инициализирован. Нажмите 'Обновить итератор'",
                    "Ошибка", JOptionPane.WARNING_MESSAGE);
            return;
        }

        if (currentIterator.hasNext()) {
            currentElement = currentIterator.next();
            currentKey = currentElement.getKey();
            currentIndex = hashTable.getIndexByKey(currentKey);
            updateIteratorStatus();
            highlightCurrentRow();
            appendToOutput("→ Переход к следующему: " + formatElementString(currentElement) + "\n");
        } else {
            showMessageDialog("Достигнут конец коллекции (end())", "Информация", JOptionPane.INFORMATION_MESSAGE);
            currentElement = null;
            currentKey = null;
            currentIndex = -1;
            updateIteratorStatus();
            appendToOutput("⚠ Достигнут конец коллекции (end())\n");
        }
    }

    private void updateIteratorStatus() {
        if (currentElement != null) {
            currentElementLabel.setText(String.format(
                    "Текущий элемент: key = \"%s\", value = \"%s\" | индекс = %d | k' = %d",
                    currentElement.getKey(),
                    currentElement.getValue(),
                    currentIndex,
                    KeyTransformer.transform(currentElement.getKey())
            ));
            currentElementLabel.setForeground(COLOR_SUCCESS);
            iteratorStatusLabel.setText("Состояние итератора: установлен");
            iteratorStatusLabel.setForeground(COLOR_SUCCESS);
        } else if (hashTable != null && hashTable.size() > 0) {
            currentElementLabel.setText("Текущий элемент: (конец коллекции - end())");
            currentElementLabel.setForeground(COLOR_WARNING);
            iteratorStatusLabel.setText("Состояние итератора: не установлен (end)");
            iteratorStatusLabel.setForeground(COLOR_WARNING);
        } else {
            currentElementLabel.setText("Текущий элемент: (нет)");
            currentElementLabel.setForeground(COLOR_ERROR);
            iteratorStatusLabel.setText("Состояние итератора: не установлен (коллекция пуста)");
            iteratorStatusLabel.setForeground(COLOR_ERROR);
        }
    }

    private void updateTableDisplay() {
        tableModel.setRowCount(0);
        if (hashTable == null || hashTable.size() == 0) return;

        List<HashTable.Entry<String, String>> entries = hashTable.getAllEntries();
        int num = 1;
        for (HashTable.Entry<String, String> entry : entries) {
            int idx = hashTable.getIndexByKey(entry.getKey());
            long transformedKey = KeyTransformer.transform(entry.getKey());
            int hashIndex = (int)(transformedKey % hashTable.capacity());
            tableModel.addRow(new Object[]{num++, idx, entry.getKey(), entry.getValue(), transformedKey, hashIndex});
        }
        highlightCurrentRow();
    }

    private void highlightCurrentRow() {
        if (currentElement == null || hashTable == null) {
            iteratorTable.clearSelection();
            return;
        }

        for (int i = 0; i < tableModel.getRowCount(); i++) {
            String key = (String) tableModel.getValueAt(i, 2);
            if (key != null && key.equals(currentKey)) {
                iteratorTable.setRowSelectionInterval(i, i);
                iteratorTable.scrollRectToVisible(iteratorTable.getCellRect(i, 0, true));
                return;
            }
        }
        iteratorTable.clearSelection();
    }

    private void performFullTraversal() {
        if (hashTable == null || hashTable.size() == 0) {
            iteratorOutput.setText("Коллекция пуста.");
            return;
        }

        StringBuilder sb = new StringBuilder();
        sb.append("=".repeat(80)).append("\n");
        sb.append("ПОЛНЫЙ ОБХОД КОЛЛЕКЦИИ (foreach)\n");
        sb.append("=".repeat(80)).append("\n");
        sb.append(String.format("%-4s | %-5s | %-12s | %-12s | %-12s\n",
                "№", "Индекс", "Ключ", "Значение", "Хеш-индекс"));
        sb.append("-".repeat(80)).append("\n");

        int count = 0;
        for (HashTable.Entry<String, String> entry : hashTable) {
            count++;
            int idx = hashTable.getIndexByKey(entry.getKey());
            long transformed = KeyTransformer.transform(entry.getKey());
            int hashIdx = (int)(transformed % hashTable.capacity());
            sb.append(String.format("%-4d | %-5d | %-12s | %-12s | %-12d\n",
                    count, idx, entry.getKey(), entry.getValue(), hashIdx));
        }
        sb.append("-".repeat(80)).append("\n");
        sb.append("Всего элементов: ").append(count).append("\n");
        sb.append("=".repeat(80)).append("\n");
        sb.append("\nПримечание: Удаление через Iterator.remove() корректно обновляет итератор.\n");

        iteratorOutput.setText(sb.toString());
        resetIterator();
    }

    private void loadSampleData() {
        if (hashTable == null) return;
        hashTable.clear();

        String[] keys = {"APPLE", "BANANA", "CHERRY", "DATE", "ELDERBERRY", "FIG", "GRAPE"};
        String[] values = {"Яблоко", "Банан", "Вишня", "Финик", "Бузина", "Инжир", "Виноград"};

        for (int i = 0; i < keys.length; i++) {
            hashTable.insert(keys[i], values[i]);
        }

        updateTableDisplay();
        resetIterator();
        appendToOutput("📋 Загружено " + keys.length + " элементов\n");
    }

    private String formatElementString(HashTable.Entry<String, String> entry) {
        return String.format("\"%s\" → \"%s\"", entry.getKey(), entry.getValue());
    }

    private void appendToOutput(String text) {
        SwingUtilities.invokeLater(() -> {
            iteratorOutput.append(text);
            iteratorOutput.setCaretPosition(iteratorOutput.getDocument().getLength());
        });
    }

    private void showMessageDialog(String message, String title, int messageType) {
        SwingUtilities.invokeLater(() -> JOptionPane.showMessageDialog(this, message, title, messageType));
    }

    public void setHashTable(HashTable<String, String> table) {
        this.hashTable = table;
        updateTableDisplay();
        resetIterator();
    }

    public HashTable<String, String> getHashTable() {
        return hashTable;
    }
}